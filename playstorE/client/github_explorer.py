...
class GitHubExplorer:
    ...
    async def analyze_repo(self, repo_full_name: str) -> RepositoryAnalysis:
        ...
        # Check safety
        is_safe, risk_score = self._assess_safety(files, readme)
    ...
    def _detect_ports(self, readme: str) -> List[int]:
        ...
        pattern = r"(?:port|:)\s*(\d{4,5})"
        matches = re.findall(pattern, readme.lower())
        return [int(m) for m in matches if 1 <= int(m) <= 65535]
    
    def _assess_safety(self, files: List[str], readme: str) -> Tuple[bool, float]:
        ...
        has_ci = ".github/workflows" in files or ".gitlab-ci.yml" in files
